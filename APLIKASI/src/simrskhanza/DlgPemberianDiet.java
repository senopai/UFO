...
public class DlgPemberianDiet extends javax.swing.JDialog {
    ...
    private String tanggalTujuanSalin = "";
    ...
    public DlgPemberianDiet(java.awt.Frame parent, boolean modal) {
        ...
        tabMode=new DefaultTableModel(null,new Object[]{
                "","No.Rawat","Nama Pasien","Kamar","Tanggal","Waktu","Jam","Diet","Diagnosa","Kode Kamar","Kode Diet","Alergi"
            }){
        ...
        tbDataDiet.getColumnModel().getColumn(0).setHeaderRenderer(new CheckBoxHeader());
        tbDataDiet.getTableHeader().addMouseListener(new java.awt.event.MouseAdapter() {
            @Override
            public void mouseClicked(java.awt.event.MouseEvent evt) {
                if (tbDataDiet.columnAtPoint(evt.getPoint()) == 0) {
                    statCheck = !statCheck;
                    for (int i = 0; i < tbDataDiet.getRowCount(); i++) {
                        tbDataDiet.setValueAt(statCheck, i, 0);
                    }
                    tbDataDiet.getTableHeader().repaint();
                }
            }
        });
    ...
    private void initComponents() {
        ...
        MnSalinJadwalDiet = new javax.swing.JMenuItem();
        ...
        MnSalinJadwalDiet.setBackground(new java.awt.Color(255, 255, 254));
        MnSalinJadwalDiet.setFont(new java.awt.Font("Tahoma", 0, 11)); // NOI18N
        MnSalinJadwalDiet.setForeground(java.awt.Color.darkGray);
        MnSalinJadwalDiet.setIcon(new javax.swing.ImageIcon(getClass().getResource("/picture/category.png"))); // NOI18N
        MnSalinJadwalDiet.setText("Salin Jadwal Diet");
        MnSalinJadwalDiet.setName("MnSalinJadwalDiet"); // NOI18N
        MnSalinJadwalDiet.setPreferredSize(new java.awt.Dimension(150, 28));
        MnSalinJadwalDiet.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                MnSalinJadwalDietActionPerformed(evt);
            }
        });
        jPopupMenu1.add(MnSalinJadwalDiet);
        ...
        jLabel24.setBounds(750, 40, 50, 23);
        ...
        Alergi.setBounds(810, 40, 150, 23);
    ...
    private void BtnSimpanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnSimpanActionPerformed
        if(TabRawat.getSelectedIndex()==0){
            /*if(TNoRw.getText().trim().equals("")||TPasien.getText().trim().equals("")){
                ...
                if(Sequel.menyimpantf("detail_beri_diet","'"+TNoRw.getText()+"','"+Kamar.getText()+"','"+ Alergi.getText()+"','"+
                        Valid.SetTgl(DTPTgl.getSelectedItem()+"")+"','"+
                        WaktuDiet.getText()+"','"+
                        KdDiet.getText()+"'","data")==true){
                    tabMode.addRow(new String[]{
                        TNoRw.getText(),TPasien.getText(),Ruang.getText(),Alergi.getText(),Valid.SetTgl(DTPTgl.getSelectedItem()+""),WaktuDiet.getText(),JamDiet.getText(),NmDiet.getText(),"-",Kamar.getText(),KdDiet.getText()
                    });
        ...
        }else if(KdDiet.getText().trim().equals("")){
            Valid.textKosong(KdDiet,"Diet");
        }else if(WaktuDiet.getText().trim().equals("")){
            Valid.textKosong(WaktuDiet,"Waktu");
        }else{
            String tgl = Valid.SetTgl(DTPTgl.getSelectedItem()+"");
            if(tgl.isEmpty() || tgl.equalsIgnoreCase("tidak ada")){
                JOptionPane.showMessageDialog(null, "Tanggal tidak valid!");
                return;
            }
            Sequel.menyimpan("detail_beri_diet","?,?,?,?,?,?","Diet Pasien",6,new String[]{
                TNoRw.getText(),Kamar.getText(),tgl,WaktuDiet.getText(),KdDiet.getText(),Alergi.getText()
            });
            tampil();
            emptTeks();
        }
    ...
    private void BtnHapusActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnHapusActionPerformed
        if(TabRawat.getSelectedIndex()==0){
            ...
            // 2. SKENARIO 1: HAPUS MASSAL (Jika ada checklist)
            if (jumlahPilih > 0) {
                hapusMassal(); // Langsung panggil method di atas
                
            } else {
                // 3. SKENARIO 2: HAPUS SATUAN (Tanpa Konfirmasi)
                if(tbDataDiet.getSelectedRow() != -1){
                    
                    String noRawat = tbDataDiet.getValueAt(tbDataDiet.getSelectedRow(), 1).toString();
                    String tglDiet = tbDataDiet.getValueAt(tbDataDiet.getSelectedRow(), 4).toString();
                    String waktu   = tbDataDiet.getValueAt(tbDataDiet.getSelectedRow(), 5).toString();
                    String kdDiet  = tbDataDiet.getValueAt(tbDataDiet.getSelectedRow(), 10).toString();

                    // Langsung Eksekusi Hapus (Menghapus bagian JOptionPane.showConfirmDialog)
                    if(Sequel.queryutf("delete from detail_beri_diet " +
                            "where no_rawat='"+noRawat+"' " +
                            "and tanggal='"+tglDiet+"' " +
                            "and waktu='"+waktu+"' " +
                            "and kd_diet='"+kdDiet+"'")==true){

                        tampil(); // Refresh data dari database
                        emptTeks();
                        // JOptionPane.showMessageDialog(null, "Data berhasil dihapus."); // Opsional
                    }
                    
                } else {
                    JOptionPane.showMessageDialog(null,"Silahkan pilih data yang mau dihapus.\nCentang checkbox ATAU klik salah satu baris tabel.");
                }
            }
        }else{
            JOptionPane.showMessageDialog(null,"Silahkan buka tab Data Diet Pasien");
        }
        ...
        if(TabRawat.getSelectedIndex()==0){
            ...
            if(tabMode.getRowCount()==0){
                ...
                Valid.MyReportqry("rptBrDiet.jasper","report","::[ Data Pemberian Diet ]::",
                    ...
                    "detail_beri_diet.tanggal,detail_beri_diet.waktu,jam_diet_pasien.jam,diet.nama_diet,detail_beri_diet.kd_kamar,detail_beri_diet.kd_diet detail_beri_diet.alergi" +
...
private void BtnSeek1KeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_BtnSeek1KeyPressed
        Valid.pindah(evt,KdDiet,Alergi);
...
private void ChkInputActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ChkInputActionPerformed
  isForm();                
    ...
    private void MnLabelDietActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_MnLabelDietActionPerformed
        if(TPasien.getText().trim().equals("")){
            ...
            Valid.MyReportqry("rptLabelDiet.jasper","report","::[ Label Diet ]::",
                "select detail_beri_diet.no_rawat,reg_periksa.no_rkm_medis,pasien.nm_pasien,pasien.tgl_lahir, detail_beri_diet.alergi, " +
    ...
    private void AlergiKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_AlergiKeyPressed
        Valid.pindah(evt,BtnSeek1,BtnSimpan);
    }//GEN-LAST:event_AlergiKeyPressed

    private void MnSalinJadwalDietActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_MnSalinJadwalDietActionPerformed
        int jumlahPilih = 0;
        for (int i = 0; i < tabMode.getRowCount(); i++) {
            if (tabMode.getValueAt(i, 0).toString().equals("true")) {
                jumlahPilih++;
        ...
        if (jumlahPilih == 0) {
            JOptionPane.showMessageDialog(null, "Silahkan centang pasien yang akan disalin jadwal dietnya.");
            return;
        }
        java.util.Calendar cal = java.util.Calendar.getInstance();
        cal.add(java.util.Calendar.DAY_OF_MONTH, 1);
        Date tglBesok = cal.getTime();

        widget.Tanggal TanggalTujuan = new widget.Tanggal();
        TanggalTujuan.setForeground(new java.awt.Color(50, 70, 50));
        TanggalTujuan.setModel(new javax.swing.DefaultComboBoxModel(new String[] { 
            new java.text.SimpleDateFormat("dd-MM-yyyy").format(tglBesok) 
        }));
        TanggalTujuan.setDisplayFormat("dd-MM-yyyy");
        TanggalTujuan.setDate(tglBesok);
        TanggalTujuan.setPreferredSize(new Dimension(140,23));

        int jawab = JOptionPane.showConfirmDialog(null, 
            new Object[]{"Pilih Tanggal Tujuan Salin :", TanggalTujuan},
            "Salin Jadwal Diet", JOptionPane.OK_CANCEL_OPTION);

        if (jawab == JOptionPane.OK_OPTION) {
            tanggalTujuanSalin = Valid.SetTgl(TanggalTujuan.getSelectedItem().toString());
    ...
    }//GEN-LAST:event_MnSalinJadwalDietActionPerformed
    
    private void simpanMassal() {
        if (jamdiet.getTable().getSelectedRow() == -1) {
            JOptionPane.showMessageDialog(null, "Silahkan pilih waktu diet (Pagi/Siang/Sore) terlebih dahulu pada tabel.");
            return;
        }
        String targetWaktu = jamdiet.getTable().getValueAt(jamdiet.getTable().getSelectedRow(), 0).toString();
        String tglSimpan = tanggalTujuanSalin;
        int sukses = 0;
        int gagal = 0;
        int duplikat = 0;
        String sqlSimpan = "INSERT IGNORE INTO detail_beri_diet (no_rawat, kd_kamar, tanggal, waktu, kd_diet, alergi) VALUES (?,?,?,?,?,?)";
        try {
            PreparedStatement psSimpan = koneksi.prepareStatement(sqlSimpan);
            for (int i = 0; i < tabMode.getRowCount(); i++) {
                if (tabMode.getValueAt(i, 0).toString().equals("true")) {
                    try {
                        String noRawat = tabMode.getValueAt(i, 1).toString();
                        String kdDiet = tabMode.getValueAt(i, 10).toString();
                        
                        String alergiPasien = "";
                        if (tabMode.getValueAt(i, 11) != null) {
                            alergiPasien = tabMode.getValueAt(i, 11).toString();
                        }
                        String kdKamarUpdate = Sequel.cariIsi(
                                "select kd_kamar from kamar_inap where no_rawat=? order by tgl_masuk desc, jam_masuk desc limit 1",
                                noRawat
                        );
                        if (kdKamarUpdate.equals("")) {
                            kdKamarUpdate = tabMode.getValueAt(i, 9).toString();
                        }
                        psSimpan.setString(1, noRawat);
                        psSimpan.setString(2, kdKamarUpdate);
                        psSimpan.setString(3, tglSimpan);   
                        psSimpan.setString(4, targetWaktu); 
                        psSimpan.setString(5, kdDiet);
                        psSimpan.setString(6, alergiPasien);
                        
                        int hasil = psSimpan.executeUpdate();
                        if (hasil > 0) {
                            sukses++;
                        } else {
                            duplikat++;
                        ...
                        System.out.println("Gagal simpan row " + i + ": " + e);
                        gagal++;
                    }
                }
            }
            ...
            System.out.println("Error Prepare: " + e);
        }
        tampil();
    }
    
    private void hapusMassal() {
        int jumlahPilih = 0;
        for (int i = 0; i < tabMode.getRowCount(); i++) {
            if (tabMode.getValueAt(i, 0).toString().equals("true")) {
                jumlahPilih++;
        ...
        if (jumlahPilih > 0) {
            ...
            String sql = "delete from detail_beri_diet where no_rawat=? and tanggal=? and waktu=? and kd_diet=?";

            try {
                PreparedStatement psHapus = koneksi.prepareStatement(sql);

                for (int i = 0; i < tabMode.getRowCount(); i++) {
                    if (tabMode.getValueAt(i, 0).toString().equals("true")) {
                        try {
                            String noRawat = tabMode.getValueAt(i, 1).toString();
                            String tglDiet = tabMode.getValueAt(i, 4).toString(); 
                            String waktu = tabMode.getValueAt(i, 5).toString();
                            String kdDiet = tabMode.getValueAt(i, 10).toString(); 

                            psHapus.setString(1, noRawat);
                            psHapus.setString(2, tglDiet);
                            psHapus.setString(3, waktu);
                            psHapus.setString(4, kdDiet);
                            
                            psHapus.executeUpdate();
                            sukses++;
                            
                        } catch (Exception e) {
                            System.out.println("Gagal Hapus Row " + i + ": " + e);
                            gagal++;
                        }
                    }
                }
                ...
                System.out.println("Error Koneksi Delete: " + e);
            }
    ...
    */
    ...
    private javax.swing.JMenuItem MnSalinJadwalDiet;
    ...
    public void tampil() {   
        try{
            ...
            try {
                ...
                rs=ps.executeQuery();
                while(rs.next()){
                    tabMode.addRow(new Object[]{ // Gunakan Object[] bukan String[]
                        Boolean.FALSE, // Kolom 0: Default tidak tercentang
                        rs.getString(1), // No Rawat
                        rs.getString(2)+" "+rs.getString(3), // Nama Pasien
                        rs.getString(4), // Kamar
                        rs.getString(5), // Tanggal
                        rs.getString(6), // Waktu (Pagi/Siang/Sore)
                        rs.getString(7), // Jam
                        rs.getString(8), // Nama Diet
    ...
    public void tampil2() {   
        try{
            ...
            ps2=koneksi.prepareStatement(
                ...
                "inner join bangsal on kamar.kd_bangsal=bangsal.kd_bangsal " +
                "where detail_beri_diet.tanggal between ? and ? and detail_beri_diet.waktu like ? and bangsal.nm_bangsal like ? "+
                (TCari.getText().trim().equals("")?"":"and (detail_beri_diet.no_rawat like ? or reg_periksa.no_rkm_medis like ? or pasien.nm_pasien like ? or diet.nama_diet like ?) ")+
            ...
            try {
                ...
                while(rs.next()){
                     int jumlahPerBaris = rs.getInt(2);
                     total = total + jumlahPerBaris;
                     tabMode2.addRow(new String[]{i+"",rs.getString(1),String.valueOf(jumlahPerBaris)});
                     i++;                   
                }

                if (total > 0) {
                    tabMode2.addRow(new String[]{"", "TOTAL", String.valueOf(total)});
                }
    ...
    private void getData() {
        if(tbDataDiet.getSelectedRow()!= -1){
            ...
            TNoRw.setText(tbDataDiet.getValueAt(tbDataDiet.getSelectedRow(),1).toString()); 
            TPasien.setText(tbDataDiet.getValueAt(tbDataDiet.getSelectedRow(),2).toString());    
            Ruang.setText(tbDataDiet.getValueAt(tbDataDiet.getSelectedRow(),3).toString());          
            WaktuDiet.setText(tbDataDiet.getValueAt(tbDataDiet.getSelectedRow(),5).toString());
            JamDiet.setText(tbDataDiet.getValueAt(tbDataDiet.getSelectedRow(),6).toString());
            NmDiet.setText(tbDataDiet.getValueAt(tbDataDiet.getSelectedRow(),7).toString());
            Kamar.setText(tbDataDiet.getValueAt(tbDataDiet.getSelectedRow(),9).toString());
            KdDiet.setText(tbDataDiet.getValueAt(tbDataDiet.getSelectedRow(),10).toString());
            Alergi.setText(tbDataDiet.getValueAt(tbDataDiet.getSelectedRow(),11).toString());
    ...
    // Class Renderer Checkbox Header
    class CheckBoxHeader extends javax.swing.JCheckBox implements javax.swing.table.TableCellRenderer {
        public CheckBoxHeader() {
            setHorizontalAlignment(javax.swing.JLabel.CENTER); // Posisi tengah
            setOpaque(false);
        ...
        public java.awt.Component getTableCellRendererComponent(
            ...
            // Set status centang sesuai variabel global statCheck
            setSelected(statCheck);
            
            // Ambil style dari header bawaan agar tampilan seragam (Background abu-abu, border, dll)
            javax.swing.JComponent component = (javax.swing.JComponent) table.getTableHeader().getDefaultRenderer()
