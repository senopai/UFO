...
public class DlgCariTagihanOperasi extends javax.swing.JDialog {
    ...
    private String Suspen_Piutang_Operasi_Ranap="",Operasi_Ranap="",Beban_Jasa_Medik_Dokter_Operasi_Ranap="",Utang_Jasa_Medik_Dokter_Operasi_Ranap="",
            ...
            finger="",kodeoperator="",nomor_implan,FileName="",kodeberkas;
    ...
    private void initComponents() {
        ...
        UploadPDF.setText("Upload PDF");
        UploadPDF.setName("UploadPDF"); // NOI18N
        UploadPDF.setPreferredSize(new java.awt.Dimension(100, 26));
        ...
        label12.getAccessibleContext().setAccessibleName("Mulai :");
...
private void MnHapusObatOperasiActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_MnHapusObatOperasiActionPerformed
    ...
    if (Laporan.getText().trim().equals("")) {
        JOptionPane.showMessageDialog(rootPane, "Laporan operasi masih kosong!");
        Laporan.requestFocus();
        return;
    }

    if (tgl2.getSelectedItem() == null || tgl3.getSelectedItem() == null) {
        JOptionPane.showMessageDialog(rootPane, "Tanggal & jam operasi belum lengkap!");
        return;
    }

    int row = tbDokter.getSelectedRow();
    if (row == -1) {
        JOptionPane.showMessageDialog(rootPane, "Silahkan pilih data operasi terlebih dahulu!");
        return;
    }

    String noRawat     = tbDokter.getValueAt(row, 1).toString();
    String tanggalLama = tbDokter.getValueAt(row, 0).toString();

    String jamMulaiBaru  = Valid.SetTgl(tgl2.getSelectedItem() + "") + " " + tgl2.getSelectedItem().toString().substring(11, 19);
    String jamSelesaiBaru = Valid.SetTgl(tgl3.getSelectedItem() + "") + " " + tgl3.getSelectedItem().toString().substring(11, 19);

    try {
        // 1. Hapus laporan lama → pakai queryu2 (bisa 2 parameter)
        Sequel.queryu2("DELETE FROM laporan_operasi WHERE no_rawat=? AND tanggal=?", 2, new String[]{noRawat, tanggalLama});

        // 2. Simpan laporan baru
        Sequel.menyimpan("laporan_operasi", "?,?,?,?,?,?,?,?,?", "Laporan Operasi", 9, new String[]{
            noRawat,
            jamMulaiBaru,           // ← dari tgl2 (jam mulai)
            PreOp.getText(),
            PostOp.getText(),
            Jaringan.getText(),
            jamSelesaiBaru,         // ← dari tgl3 (jam selesai)
            DikirimPA.getSelectedItem().toString(),
            NomorImplant.getText(),
            Laporan.getText()
        });

        // 3. Update tabel operasi biar konsisten → pakai queryu2 (3 parameter)
        Sequel.queryu2("UPDATE operasi SET tgl_operasi=? WHERE no_rawat=? AND tgl_operasi=?", 3, new String[]{jamMulaiBaru, noRawat, tanggalLama});

        JOptionPane.showMessageDialog(rootPane, "Update berhasil! Jam mulai & selesai operasi telah diperbarui.");
        tampil(); // refresh tabel

    } catch (Exception e) {
        JOptionPane.showMessageDialog(rootPane, "Error: " + e.toString());
        System.out.println("Error BtnSimpan: " + e);
    }
    ...
    private void UploadPDFActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_UploadPDFActionPerformed
        FileName = "OP_" + tbDokter.getValueAt(tbDokter.getSelectedRow(), 1).toString().replaceAll("/", "");
        CreatePDF(FileName);
